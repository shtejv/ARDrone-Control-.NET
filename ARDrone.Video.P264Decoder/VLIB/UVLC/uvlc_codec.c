...
static C_RESULT uvlc_load_stream( video_stream_t* out, video_stream_t* in )
{
  ...
  {
    out->bytes = static_cast<uint32_t*>(vp_os_realloc(out->bytes, out->size + 2048 )); // Add 2KB to internal stream
    out->size += 2048;
  }
...
C_RESULT uvlc_encode_blockline( video_controller_t* controller, const vp_api_picture_t* blockline, bool_t picture_complete )
{
  ...
  {
	uint32_t add = 32 - clz3(stream->used/controller->blockline); 	    // estimate the log2 size of a blockline in the stream
	add = 1<<(add+1);												    // major and compute addition buffer size
    stream->bytes =  static_cast<uint32_t*>(vp_os_realloc(stream->bytes, stream->size + add )); // Add some byte to internal stream
    stream->size += add;
  }
...
{
  video_codec_t* video_codec;
  vp_api_picture_t blockline = { static_cast<PixelFormat>(0) };
  int16_t *in = NULL, *out = NULL;
  int32_t num_macro_blocks = 0;
  video_macroblock_t* macroblock = NULL;
  video_picture_context_t blockline_ctx;
