...
C_RESULT vp_com_read_socket(vp_com_socket_t* socket, int8_t* buffer, int32_t* size)
{
  ...
  {
    res = VP_COM_OK;
    *size = /*read*/recv(s, reinterpret_cast<char*>(buffer), *size, flags);
    if(*size < 0)
    {
      if( errno == EAGAIN )
      {
...
C_RESULT vp_com_write_socket(vp_com_socket_t* socket, const int8_t* buffer, int32_t* size)
{
  ...
  {
    res = VP_COM_OK;
    *size = send(s, const_cast<char*>(reinterpret_cast<const char*>(buffer)), *size, flags);
    if(*size < 0)
    {
      if( errno == EAGAIN )
      {
  ...
  else
  {
    ...
    for( i = 0; i < num_server_sockets; i++ )
    {
      if(VP_FAILED( vp_com_open_socket(&server_sockets[i], NULL, NULL) ))
      {
        ...
        server_sockets[i].is_disable = static_cast<VP_COM_SOCKET_STATE>(TRUE);
      }
      else
      {
